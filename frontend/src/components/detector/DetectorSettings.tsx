 ...
 */
export function validatePollIntervals(base: number, min: number, max: number): PollIntervalErrors {
...
export function DetectorSettings({
  ...
  const settingsContent = (
    <div
      className={`${embedded ? "space-y-3" : "px-4 pb-4 space-y-3 border-t border-border-subtle pt-3"} ${disabled ? "opacity-50 pointer-events-none" : ""}`}
    ...
    >
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <label htmlFor="det-precision" className="text-xs 2xl:text-sm text-text-muted">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label htmlFor="det-cooldown" className="block text-xs 2xl:text-sm text-text-muted mb-1">
            {t("detector.cooldown")}
          </label>
          <input
            id="det-cooldown"
        ...
        <div>
          <label htmlFor="det-hits" className="block text-xs 2xl:text-sm text-text-muted mb-1">
            {t("detector.hits")}
          </label>
          <input
            id="det-hits"
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <label htmlFor="det-hysteresis" className="text-xs 2xl:text-sm text-text-muted">
      ...
      <div>
        ...
        <p
          id="det-hysteresis-mode-desc"
          className="text-[11px] leading-relaxed text-text-muted mt-0.5"
        >
          {t("detector.hysteresisModeDesc")}
        </p>
      </div>
          ...
          return (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label
                    htmlFor="det-base-poll"
                    className="block text-xs 2xl:text-sm text-text-muted mb-1"
                  >
                    {t("detector.basePoll")}
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      onUpdate({ poll_interval_ms: Number.parseInt(e.target.value, 10) || 50 })
                    }
                    className={`${inputBase} ${errs.base ? errBorder : okBorder}`}
                  />
                  {errs.base ? (
                    <p
                      id="det-base-poll-err"
                      className="text-[11px] leading-relaxed text-accent-red mt-0.5"
                    >
                      {t(errs.base, { min: minPollMs, max: maxPollMs })}
                    </p>
                  ) : (
                    <p className="text-[11px] leading-relaxed text-text-muted mt-0.5">
                      {t("detector.basePollDesc")}
                    </p>
                  )}
                </div>
                <div>
                  <label
                    htmlFor="det-min-poll"
                    className="block text-xs 2xl:text-sm text-text-muted mb-1"
                  >
                    {t("detector.minPoll")}
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      onUpdate({ min_poll_ms: Number.parseInt(e.target.value, 10) || 30 })
                    }
                  ...
                  {errs.min ? (
                    <p
                      ...
                      className="text-[11px] leading-relaxed text-accent-red mt-0.5"
                    >
                      {t(errs.min, { max: maxPollMs })}
                    </p>
                  ) : (
                    <p className="text-[11px] leading-relaxed text-text-muted mt-0.5">
                      {t("detector.minPollDesc")}
                    </p>
                  )}
                </div>
                <div>
                  <label
                    htmlFor="det-max-poll"
                    className="block text-xs 2xl:text-sm text-text-muted mb-1"
                  >
                    {t("detector.maxPoll")}
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      onUpdate({ max_poll_ms: Number.parseInt(e.target.value, 10) || 500 })
                    }
                  ...
                  {errs.max ? (
                    <p
                      ...
                      className="text-[11px] leading-relaxed text-accent-red mt-0.5"
                    >
                      {t(errs.max, { min: minPollMs })}
                    </p>
                  ) : (
                    <p className="text-[11px] leading-relaxed text-text-muted mt-0.5">
                      {t("detector.maxPollDesc")}
                    </p>
                  )}
                </div>
              </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ChevronDown
          className={`w-4 h-4 text-text-muted transition-transform ${showSettings ? "rotate-180" : ""}`}
        />
      </button>
