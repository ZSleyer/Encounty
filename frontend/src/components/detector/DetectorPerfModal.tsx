/**
 * DetectorPerfModal, a dev-only diagnostic modal that shows live runtime
...
import { Activity, Cpu, MonitorCog } from "lucide-react";
...
/** Placeholder shown when a metric is not available. */
const NO_VALUE = "–";
...
function fmtMB(mb: number): string {
  if (!Number.isFinite(mb)) return NO_VALUE;
...
export default function DetectorPerfModal({
  ...
  return (
    <ModalShell
      ...
      size="xl"
      structured
      titleIcon={<Activity className="w-5 h-5 text-accent-blue" />}
      footer={
        <p className="text-[11px] text-text-muted">
          {t("perfModal.refreshHint", { seconds: POLL_INTERVAL_MS / 1000 })}
        </p>
      }
    >
...
function ProcRow({ label, sample, highlight }: Readonly<ProcRowProps>): JSX.Element {
  return (
    <tr className={highlight ? "text-accent-blue" : undefined}>
      ...
      <td className="py-1 text-right">{sample ? fmtPct(sample.cpuPct) : NO_VALUE}</td>
      <td className="py-1 text-right">{sample ? fmtMB(sample.memMB) : NO_VALUE}</td>
    </tr>
  );
}
