/**
 * Pure mathematical functions for CPU-based NCC template matching.
 *
 * Extracted from CPUDetector.ts for independent testability. All functions
 * operate on typed arrays and have no browser API dependencies (no Canvas,
 * no WebGPU, no Video elements).
 */
 ...
 */
export function pixelDelta(a: Float32Array, b: Float32Array): number {
 ...
 */
export function histogramCorrelation(a: Float32Array, b: Float32Array): number {
  const BINS = 64;
 ...
 * pixels, 0.0 means average difference >= 128 gray levels.
 */
export function madSimilarity(a: Float32Array, b: Float32Array): number {
  const n = a.length;
  if (n === 0) return 0;
  let sum = 0;
  for (let i = 0; i < n; i++) {
...
export function matchWholeTemplate(
