...
export const FORM_CATEGORIES: DexFormCategory[] = ["regional", "mega", "gigantamax", "gender", "cosmetic", "other"];
...
export function speciesInPokedex(species: PokemonData, dex: UserPokedex, _games: GameEntry[]): boolean {
  ...
  const generation = species.id <= 151 ? 1 : species.id <= 251 ? 2 : species.id <= 386 ? 3 : species.id <= 493 ? 4 : species.id <= 649 ? 5 : species.id <= 721 ? 6 : species.id <= 809 ? 7 : species.id <= 905 ? 8 : 9;
  if (dex.generations.includes(generation)) return true;
  return dex.target_games.some((key) => species.games?.includes(key));
}

export function pokemonInPokedex(pokemon: Pokemon, dex: UserPokedex, pokedex: PokemonData[], games: GameEntry[]): boolean {
  if (dex.catch_games.length > 0 && !dex.catch_games.includes(pokemon.game)) return false;
  const species = pokedex.find((entry) => entry.canonical === pokemon.canonical_name || entry.forms?.some((form) => form.canonical === pokemon.canonical_name));
