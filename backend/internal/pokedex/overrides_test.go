...
func TestSetOverrideWithMetaStoresIt(t *testing.T) {
	...
	got, deleted, err := SetOverride(store, 25, "", "", "", true, true, meta)
	if err != nil {
...
func TestSetOverrideNilMetaOnFreshRowDefaultsEmpty(t *testing.T) {
	...
	got, _, err := SetOverride(store, 1, "", "", "", true, false, nil)
	if err != nil {
		t.Fatalf("SetOverride: %v", err)
	}
	if got.Meta != nil {
...
func TestSetOverrideNilMetaPreservesExisting(t *testing.T) {
	...
	if _, _, err := SetOverride(store, 1, "", "", "", true, false, meta); err != nil {
		t.Fatalf("initial SetOverride: %v", err)
	}

	got, deleted, err := SetOverride(store, 1, "", "", "", true, true, nil)
	if err != nil {
...
func TestSetOverrideExplicitEmptyMetaClears(t *testing.T) {
	...
	got, _, err := SetOverride(store, 1, "", "", "", true, false, &state.CatchMeta{})
	if err != nil {
...
func TestSetOverrideDeletePathSkipsMetaLookup(t *testing.T) {
	...
	_, deleted, err := SetOverride(store, 1, "", "", "", false, false, nil)
	if err != nil {
		t.Fatalf("SetOverride: %v", err)
	}
	if !deleted {
