...
var schemaV2 = []string{
	...
	`CREATE TABLE IF NOT EXISTS overlay_settings (
		...
		background_animation_config TEXT    NOT NULL DEFAULT '',
	...
	`CREATE TABLE IF NOT EXISTS detector_configs (
		...
		precision_val       REAL    NOT NULL DEFAULT 0.55,
		consecutive_hits    INTEGER NOT NULL DEFAULT 1,
		cooldown_sec        INTEGER NOT NULL DEFAULT 5,
		change_threshold    REAL    NOT NULL DEFAULT 0.15,
		poll_interval_ms    INTEGER NOT NULL DEFAULT 200,
		min_poll_ms         INTEGER NOT NULL DEFAULT 50,
		max_poll_ms         INTEGER NOT NULL DEFAULT 2000,
		adaptive_cooldown     INTEGER NOT NULL DEFAULT 0,
		adaptive_cooldown_min INTEGER NOT NULL DEFAULT 3,
		hysteresis_factor     REAL    NOT NULL DEFAULT 0.7,
		FOREIGN KEY (pokemon_id) REFERENCES pokemon(id) ON DELETE CASCADE
	)`,
	...
	`CREATE TABLE IF NOT EXISTS pokedex_species (
		...
		evolves_from_id INTEGER NOT NULL DEFAULT 0,
	...
	// user-entered overrides or break the sync outright. form_canonical=''
	// means species-level, gender='' means not gender-restricted, game=''
