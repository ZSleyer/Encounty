...
var keyMap = map[string]Key{
	...
	"8":   Key8,
	"9":   Key9,
	// Special characters — matched against e.key (single char, not uppercased).
	" ":  KeySpace,
	"'":  KeyApostrophe,
	"*":  KeyAsterisk,
	"+":  KeyPlus,
	...
	"]":  KeyBracketRight,
	"`":  KeyGrave,
	// Navigation / editing — matched against e.key (length > 1, lowercased).
	...
	"arrowright": KeyRight,
	"arrowdown":  KeyDown,
	// Numpad (browser e.key when NumLock is on uses same chars as main keys;
	// these entries handle the case where the frontend explicitly labels them).
